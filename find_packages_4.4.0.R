...
R_VERSION <- "4.4.0"
BIOC_VERSION <- "3.19"
ROCKER_DATE <- "2024-05-17"
pkgs <- installed.packages(paste0(
    '/system/linux/lib/R-22.04/', R_VERSION, '/x86_64/site-library'))
all_bioc_pkgs <- available.packages(repos = paste0('https://bioconductor.org/packages/', BIOC_VERSION, '/bioc'))
all_cran_pkgs <- available.packages(repos = paste0('https://packagemanager.posit.co/cran/__linux__/jammy/', ROCKER_DATE))
...
install_later <- c('ggm','gRbase','rags2ridges')   # needs graph installed (which comes from bioconductor)
