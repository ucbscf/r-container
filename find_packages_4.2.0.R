...
R_VERSION <- "4.2.0"
BIOC_VERSION <- "3.15"
MRAN_DATE <- "2022-05-09"
pkgs <- installed.packages(paste0(
    '/system/linux/lib/R-20.04/', R_VERSION, '/x86_64/site-library'))
all_bioc_pkgs <- available.packages(repos = paste0('https://bioconductor.org/packages/', BIOC_VERSION, '/bioc'))
all_cran_pkgs <- available.packages(repos = paste0('https://mran.microsoft.com/snapshot/', MRAN_DATE))
...
install_later <- c('ggm')   # needs graph installed (which comes from bioconductor)
